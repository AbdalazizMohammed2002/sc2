  ...
  const columns: ColumnDef<Student>[] = [
    {
      ...
      header: "Étudiant",
      accessorFn: (student) =>
        `${student.lastName} ${student.firstName} ${student.email ?? ""}`.trim(),
      cell: ({ row }) => {
    ...
    },
    {
      ...
      header: "Niveau",
      cell: ({ row }) => (
        <Badge variant="outline">{levelLabels[row.original.level]}</Badge>
      ),
    },
    {
      id: "class",
      header: "Classe",
      accessorFn: (student) => student.class?.name ?? "",
      cell: ({ row }) => (
        <span className="text-muted-foreground">
          {row.original.class?.name || "Non assigné"}
        </span>
      ),
    },
    {
      ...
      header: "Genre",
      cell: ({ row }) => (
        <span>{row.original.gender === "MALE" ? "Masculin" : "Féminin"}</span>
      ),
    },
    {
      accessorKey: "status",
