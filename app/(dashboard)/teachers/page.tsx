  ...
  const columns: ColumnDef<Teacher>[] = [
    {
      ...
      header: "Enseignant",
      accessorFn: (teacher) =>
        `${teacher.lastName} ${teacher.firstName} ${teacher.qualification ?? ""}`.trim(),
      cell: ({ row }) => {
    ...
    },
    {
      id: "contact",
      header: "Contact",
      accessorFn: (teacher) => `${teacher.email} ${teacher.phone ?? ""}`.trim(),
      cell: ({ row }) => {
        const teacher = row.original;
        return (
    ...
    },
    {
      id: "subjects",
      header: "Matières",
      accessorFn: (teacher) => teacher.subjects?.map((subject) => subject.name).join(" ") ?? "",
      enableSorting: false,
      cell: ({ row }) => (
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="font-medium">
          {row.original.salary?.toLocaleString("fr-DZ")} DA
        </span>
      ),
    },
    {
